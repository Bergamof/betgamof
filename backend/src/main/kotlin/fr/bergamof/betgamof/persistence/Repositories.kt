...
class BankrollRepository(
    ...
    fun findAll(): List<Bankroll> = transaction(db) { BankrollsTable.selectAll().orderBy(BankrollsTable.id).map(::toBankroll) }
    ...
    fun delete(id: Long): Boolean = transaction(db) { BankrollsTable.deleteWhere { BankrollsTable.id eq id } > 0 }
...
class BetRepository(
    ...
    fun find(id: Long): Bet? = findAll().firstOrNull { it.id == id }
    ...
    fun delete(id: Long): Boolean = transaction(db) { BetsTable.deleteWhere { BetsTable.id eq id } > 0 }
...
class MontanteRepository(
    ...
    fun findAll(): List<Montante> = transaction(db) { MontantesTable.selectAll().orderBy(MontantesTable.id).map(::toMontante) }
...
class RuleRepository(
    ...
    fun delete(id: Long): Boolean = transaction(db) { DisciplineRulesTable.deleteWhere { DisciplineRulesTable.id eq id } > 0 }
}
